...
#Only one wave in NL
wave1beginings <- c("ON" = 11, "AB" = 2, "QC" = 5, "BC" = 1, "SK" = 5, "MB" = 1, "NL" = 1, "NB" = 3, "NS" = 1, "PEI" = 1, "YT" = 20, "NT" = 1, "NU" = NA)
wave1endings <- c("ON" = 77 , "AB" = 38, "QC" = 29, "BC" = 9, "SK" = 16, "MB" = 16, "NL" = 7, "NB" = 23, "NS" = 34, "PEI" = 12, "YT" = 29, "NT" = 23, "NU" = NA)
...
egfs <- sapply(names(splitintervalCases)[names(splitintervalCases) != "NU"], function(provinceName){
  ...
  if (provinceName %in% c("QC", "BC", "NL")){
    ...
    theSecond <- (egf_init(cases = splitintervalCases$QC$intervalCases, date = anydate(splitintervalCases$QC$Date), first = 165, peak = 212))
      ...
      theSecond <- egf_init(date = dates, cases = provincetimeseries, peak = 230)
    }
    else{
  ...
  else if (provinceName %in% c("NB", "NS")){
  ...
  ##We should the epigrowthfit objects at the same time.
...
##Make the table.
make_table <- function(egf_df){
  return(data.frame("Province" = as.vector(colnames(egf_df)),
                         "Exponential Growth Rate" = sapply(egf_df, function(x){
                           return(x$theta0[1])}),
                         "Doubling Time"= sapply(egf_df, epigrowthfit::compute_doubling_time),
                         "Reproduction Number" = sapply(1:length(egf_df), function(x){return(1)})))
}

options(omitlatexcom = TRUE)
Hmisc::latex(make_table(egfs), title = "Parameter estimates from fits for both waves", rowname = "", file = "")
```
...
i <- 1
while (i <= length(egfs)){
  plot(egfs[[1, colnames(egfs[i])]])
  plot(egfs[[2, colnames(egfs[i])]])
  i <- i+ 1
}
