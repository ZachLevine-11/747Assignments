...
splitintervalCases <- lapply(as.vector(unique(allcases$Province)), function(provinceName){
  ...
  ##Remove the weekends.
  intervalcasesdf <- intervalcasesdf[!(is.weekend(anytime::anydate(intervalcasesdf$Date)) & thefaultyWeekends),]
...
egfs <- sapply(names(splitintervalCases)[names(splitintervalCases) != "NU"], function(provinceName){
  ...
  theLast <- which.max(provincetimeseries[1:as.integer(length(provincetimeseries)/2)]) 
  if (provinceName == "NL"){
      theLast <- theLast + 1
  }
  else{
  }
  return(list("first" = egf_init(date = dates, cases = provincetimeseries, first = 1, last = theLast),
              "second" = egf_init(date = dates, cases = provincetimeseries, first = beginingwave2, last = length(provincetimeseries))
              )
         )
  })
fittingWindow <- provincetimeseries[which.max(provincetimeseries[1:as.integer(length(provincetimeseries)/2)]) + 1:length(provincetimeseries)]
