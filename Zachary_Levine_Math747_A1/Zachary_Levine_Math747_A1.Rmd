...
splitintervalCases <- lapply(as.vector(unique(allcases$Province)), function(provinceName){
  ...
  leenumbers <- c()
  leefunction <- function(provinceName){
    i <- 1
    ournumber <- 0 
      ...
      if (is.weekend(anytime::anydate(intervalcasesdf[[i, "Date"]]))){
       ##Is a Saturday.
        if (i + 2 <= nrow(intervalcasesdf)){
                if (intervalcasesdf[[i, "intervalCases"]] == 0 & intervalcasesdf[[i + 1, "intervalCases"]] && intervalcasesdf[[i+2, "intervalCases"]]){
  ...
  leenumbers <- c(leenumbers, leefunction(provinceName))
