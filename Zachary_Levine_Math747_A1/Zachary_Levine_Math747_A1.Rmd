...
##Override window fitting with peak selection to produce better fits for a few regions
  if (provinceName %in% c("QC", "NL", "SK", "NB", "NS", "NT", "PEI")){
    if (provinceName == "NL"){
      thePeak <- 145
    }
    ...
    theSecond <- egf_init(date = dates, cases = provincetimeseries, distr = theDist, peak = thePeak)
  }
  else{
    ##Otherwise set the fitting window manulaly
